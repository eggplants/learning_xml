  ...
  <xsl:template match="/">
    <html lang="ja">
      ...
      <body>
        ...
        <div><xsl:value-of select="count(books/item)"/>
          件のメタデータ
          </div>
        <hr/>
      ...
      <table>
        ...
        <tr>
          ...
          <td>
            ...
            <button><xsl:attribute name="onclick">openWindow('<xsl:value-of select="title"/>',`<xsl:value-of select="description"/>`)</xsl:attribute>
             別ウィンドウで読む
            </button>
          </td>
        </tr>
